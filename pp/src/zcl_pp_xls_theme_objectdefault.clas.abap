CLASS zcl_pp_xls_theme_objectdefault DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_pp_xls_theme_objectdefault IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF objectdefaults IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = zcl_pp_xls_theme=>c_theme_prefix
                                                                name   = zcl_pp_xls_theme=>c_theme_object_def
                                                                parent = lo_theme ).
    ELSE.
  ...
  METHOD load.
    objectdefaults = zcl_pp_xls_common=>clone_ixml_with_namespaces( io_object_def ).
  ENDMETHOD.                    "load
ENDCLASS.
